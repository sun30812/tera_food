...
void main() {
  ...
  test('모든 음식점 목록을 정상적으로 가져오는지 테스트', () async {
    RestaurantProvider provider = await RestaurantProvider.getSampleInstance();
    expect(
      provider.foods(FoodKind.all).any((element) => element.foodName == 'CU'),
      true,
    );
    expect(
      provider
          .foods(FoodKind.all)
          .any((element) => element.foodName == '한솥 도시락'),
      true,
    );
    expect(
      provider
          .foods(FoodKind.all)
          .any((element) => element.foodName == '고스락 칼국수'),
      true,
    );
    expect(
      provider
          .foods(FoodKind.all)
          .any((element) => element.foodName == '뉴욕 버거'),
      true,
    );
    expect(
      provider.foods(FoodKind.all).any((element) => element.foodName == '명백집'),
      true,
    );
    expect(
      provider
          .foods(FoodKind.all)
          .any((element) => element.foodName == '포케올데이'),
      true,
    );
  });
